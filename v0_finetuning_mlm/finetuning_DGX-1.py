# Code for finetuning on DGX-1 (on 8 V100 GPUs) / Adjusted Code for CUDA 11.0 
...
# 3. Wrap ESM for MaskedLM using its hidden representations
...
wrapped = ESMForMaskedLM(esm_model=esm_model, vocab_size=vocab_size)
...
wrapped.config = SimpleNamespace(**config_dict)
wrapped.config.to_dict = lambda: config_dict

def _passthrough_prepare(self, input_ids, **kwargs):
    return {"input_ids": input_ids, **kwargs}

wrapped.prepare_inputs_for_generation = types.MethodType(
    _passthrough_prepare, wrapped
)
...
model = get_peft_model(wrapped, lora_config)
model.print_trainable_parameters()

# 6. Data collator: pad & mask
